		...
		getSpeciesInfo(pokemonName).then((speciesData) => {
			...
			getTypeData(data.types[0].type.url)
				...
				.then(() => {
					...
					// Text-to-Speech
					if (data.types.length > 1) {
						textToSpeech(pokemonName + ", the " + type1 + " and " + type2 + " type pokemon. " + decription);
					} else {
						textToSpeech(pokemonName + ", the " + type1 + "type pokemon. " + decription);
					}
				});
			...
			data.abilities.forEach((element) => {
				...
				document.getElementById("abilities").textContent += titleCase(str) + "\n";
			});

			//Habitat
			document.getElementById("habitat").textContent = capitalize(data.habitat.name);
			...
			datasets: [
				{
					...
					backgroundColor: ["rgba(255,137,180, 0.5)", "rgba(44, 130, 201, 0.5)"],
...
function replaceButtonColor(id, type) {
	...
	if (!textBlack.includes(type)) {
		return document.getElementById(id).className.replace("bg-red-700", colors[type]);
	} else {
		return document.getElementById(id).className.replace("bg-red-700 text-white", `${colors[type]} text-black`);
	}
}
